...
class InquirerControl(FormattedTextControl):
    def __init__(self, choices, target, **kwargs):
        self.selected_option_index = 0
        self.answered = False
        self.choices = choices
        self._init_choices(choices, target)
        super().__init__(self._get_choice_tokens, **kwargs)

    def _init_choices(self, choices, target):
        ...
        for i, c in enumerate(choices):
            if isinstance(c, Separator):
                ...
                if searching_first_choice:
                    ...
                    searching_first_choice = False
                if target and (target == i or target == c):
                    self.selected_option_index = i  # target choice exists
                    searching_first_choice = False
...
def question(message, **kwargs):
    ...
    ic = InquirerControl(choices, target=target)
    ...
    def move_cursor_down(event):
        def _next():
            selected_choice = ic.choices[ic.selected_option_index]
            del ic.choices[ic.selected_option_index]
            ic.selected_option_index = (
                (ic.selected_option_index + 1) % (ic.choice_count + 1))
            ic.choices.insert(ic.selected_option_index, selected_choice)
        _next()
    ...
    def set_answer(event):
        ic.answered = True
        event.app.exit(result=list(map(lambda x : x[1], ic.choices)))
